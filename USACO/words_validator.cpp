...
int main(int argc, char** argv) {
    ...
    len[0] = 0;
    set<pair<int, char>> appeared;
    for (int i = 1; i <= n; i++) {
        p[i] = gi(0, i - 1);
        gs();
        c[i] = in.get_char();
        fassert(c[i] >= 'a' && c[i] <= 'd', "Letter out of range");
        fassert(!appeared.count({p[i], c[i]}), "Parent-character pair not unique");
        appeared.insert({p[i], c[i]});
        gs();
        int cnt = gi(0, rem);
        ge();

        len[i] = len[p[i]] + 1;
        batch1_tot += len[i] * cnt;
        rem -= cnt;
    }
